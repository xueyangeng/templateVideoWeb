...
this.onmessage = function(e) {
  ...
  try {
    if (e.data.cmd === "load") {
      ...
      Module["registerPthreadPtr"](threadInfoStruct,
      /*isMainBrowserThread=*/
      0,
      /*isMainRuntimeThread=*/
      0);
      ...
      try {
        ...
        if (ex === "Canceled!") {
          ...
          Atomics.store(Module["HEAPU32"], (threadInfoStruct + 4) >>
          /*C_STRUCTS.pthread.threadExitCode*/
          2, (ex instanceof Module["ExitStatus"]) ? ex.status: -2);
          /*A custom entry specific to Emscripten denoting that the thread crashed.*/
          Atomics.store(Module["HEAPU32"], (threadInfoStruct + 0) >>
          /*C_STRUCTS.pthread.threadStatus*/
          2, 1);
          Module["_emscripten_futex_wake"](threadInfoStruct + 0,
          /*C_STRUCTS.pthread.threadStatus*/
          2147483647);
